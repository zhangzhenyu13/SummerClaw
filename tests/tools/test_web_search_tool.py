...
from summerclaw.agent.tools.web import WebSearchTool
...
async def test_brave_search(monkeypatch):
    ...
    tool = _tool(provider="brave", api_key="brave-key")
    result = await tool.execute(query="summerclaw", count=1)
...
async def test_duckduckgo_search(monkeypatch):
    ...
    monkeypatch.setattr("summerclaw.agent.tools.web.DDGS", MockDDGS, raising=False)
    import summerclaw.agent.tools.web as web_mod
