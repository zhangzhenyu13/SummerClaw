...
from summerclaw.channels.matrix import _build_matrix_text_content

import summerclaw.channels.matrix as matrix_module
from summerclaw.bus.events import OutboundMessage
from summerclaw.bus.queue import MessageBus
from summerclaw.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from summerclaw.channels.matrix import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("summerclaw.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "summerclaw.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("summerclaw.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "summerclaw.channels.matrix.asyncio.create_task", _fake_create_task
    )
