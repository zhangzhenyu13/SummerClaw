...
try:
    from summerclaw.channels import dingtalk
...
import summerclaw.channels.dingtalk as dingtalk_module
from summerclaw.channels.dingtalk import DingTalkChannel, NanobotDingTalkHandler
from summerclaw.channels.dingtalk import DingTalkConfig
...
async def test_handler_processes_file_message(monkeypatch) -> None:
    ...
    async def fake_download(download_code, filename, sender_id):
        return f"/tmp/summerclaw_dingtalk/{sender_id}/{filename}"
    ...
    assert "/tmp/summerclaw_dingtalk/user1/report.xlsx" in msg.content
...
async def test_download_dingtalk_file(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "summerclaw.config.paths.get_media_dir",
