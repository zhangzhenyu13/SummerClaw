...
`nanobot/cron` 是 UnionClaw/nanobot 项目的**定时任务调度模块**，负责管理 Agent 定时任务的创建、调度、执行与持久化。该模块支持三种调度模式（一次性、周期、Cron 表达式），通过磁盘持久化 + 操作日志实现多实例协调，并提供受保护的系统任务（如 Dream 记忆固化）机制。
