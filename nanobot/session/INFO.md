...
`nanobot/session` 模块负责管理 Nanobot 的对话会话（Session），提供会话的创建、加载、保存、查询以及消息历史管理功能。该模块是整个对话系统的核心基础设施，为 Agent 循环、记忆系统、命令路由等组件提供持久化的会话状态支持。
...
[Session](file:///home/bird/harness/UnionClaw/nanobot/session/manager.py#L16-L93) 是一个 `@dataclass`，表示一个对话会话。
...
`get_history` 和 `retain_recent_legal_suffix` 使用 [find_legal_message_start](file:///home/bird/harness/UnionClaw/nanobot/utils/helpers.py#L103) 工具函数确保消息历史从合法位置开始：
...
[SessionManager](file:///home/bird/harness/UnionClaw/nanobot/session/manager.py#L96-L268) 是会话管理器，负责会话的生命周期管理。
...
- 设置旧版会话目录：`~/.nanobot/sessions/`（用于迁移）
...
**测试覆盖**：[test_unified_session.py](file:///home/bird/harness/UnionClaw/tests/agent/test_unified_session.py)
...
详见：[test_session_manager_history.py](file:///home/bird/harness/UnionClaw/tests/agent/test_session_manager_history.py)

### 3. 旧版会话自动迁移

当用户升级 Nanobot 时，旧版全局会话（`~/.nanobot/sessions/`）会自动迁移到新的工作区路径。
...
**与 Consolidator 的协作**：
```python
# nanobot/memory/ 中的 Consolidator
...
[AgentLoop](file:///home/bird/harness/UnionClaw/nanobot/agent/loop.py) 是 Session 的主要消费者：
...
[CommandRouter](file:///home/bird/harness/UnionClaw/nanobot/command/router.py) 和内建命令使用 Session：
...
- [naive_memory](file:///home/bird/harness/UnionClaw/nanobot/memory/naive_memory/)
- [mastra_om_memory](file:///home/bird/harness/UnionClaw/nanobot/memory/mastra_om_memory/)
- [hindsight_memory](file:///home/bird/harness/UnionClaw/nanobot/memory/hindsight_memory/)
- [mem0v3_memory](file:///home/bird/harness/UnionClaw/nanobot/memory/mem0v3_memory/)
...
[API Server](file:///home/bird/harness/UnionClaw/nanobot/api/server.py) 使用 Session 管理 SDK 会话：
...
[test_session_manager_history.py](file:///home/bird/harness/UnionClaw/tests/agent/test_session_manager_history.py) 覆盖：
...
[test_unified_session.py](file:///home/bird/harness/UnionClaw/tests/agent/test_unified_session.py) 覆盖：
...
| 模块 | 关系 |
|------|------|
| [nanobot/agent/loop.py](file:///home/bird/harness/UnionClaw/nanobot/agent/loop.py) | 主要消费者，管理会话生命周期 |
| [nanobot/command/builtin.py](file:///home/bird/harness/UnionClaw/nanobot/command/builtin.py) | `/new`、`/stop`、`/tasks` 命令 |
| [nanobot/memory/](file:///home/bird/harness/UnionClaw/nanobot/memory/) | 所有记忆算法使用 SessionManager |
| [nanobot/api/server.py](file:///home/bird/harness/UnionClaw/nanobot/api/server.py) | SDK API 会话管理 |
| [nanobot/utils/helpers.py](file:///home/bird/harness/UnionClaw/nanobot/utils/helpers.py) | `find_legal_message_start`、`safe_filename` |
| [nanobot/config/schema.py](file:///home/bird/harness/UnionClaw/nanobot/config/schema.py) | `unified_session`、`session_ttl_minutes` 配置 |

---

## 总结

`nanobot/session` 模块是 Nanobot 对话系统的核心基础设施，提供：
