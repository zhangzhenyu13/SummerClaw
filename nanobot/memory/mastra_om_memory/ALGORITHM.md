...
> 基于 [Mastra Observational Memory](https://mastra.ai/research/observational-memory) 论文架构，为 nanobot 完全重写，零外部依赖。在 LongMemEval 基准测试中以 gpt-5-mini 达到 **94.87%** SOTA 准确率。
...
| `append_history(entry)` | 追加到 history.jsonl，返回游标 |
| `read_unprocessed_history(since_cursor)` | 按游标增量读取 |
