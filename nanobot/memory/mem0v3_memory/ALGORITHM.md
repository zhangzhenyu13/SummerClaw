...
基于 [mem0 v3](https://mem0.ai/blog/mem0-the-token-efficient-memory-algorithm)（2026 年 4 月发布），为 nanobot 完全重写，零外部依赖。
...
| 功能 | 说明 | 本项目状态 |
|------|------|-----------|
| **Reranker** | 对搜索结果二次排序（Cohere/Cross-encoder） | 不需要 — 三信号融合已经够用 |
| **Procedural Memory** | 从对话中提取程序性知识 | mem0 独立功能，不在 v3 核心 |
| **Vision/LMM** | 多模态图片→文本描述 | nanobot 通过 provider 透明处理 |
...
**结论：核心算法层面无差距。** 7 阶段管线、三信号融合检索公式、Hash 去重、防幻觉映射均与官方 100% 一致。主要差距在实体提取和词形还原这两个 NLP 预处理环节（官方用 spaCy，本项目用纯 regex），以及系统提示词的结构化程度。这些差距影响检索召回率约 5-15%，但不影响核心算法的正确性和 ADD-only + 实体链接的设计范式。对于 nanobot 的 agent 场景（对话量适中、单用户为主），当前实现已经足够。
