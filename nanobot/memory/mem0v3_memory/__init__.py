"""Mem0V3 memory algorithm — token-efficient ADD-only memory for nanobot.
...
from nanobot.memory.mem0v3_memory.dream import Mem0V3Dream
from nanobot.memory.mem0v3_memory.auto_compact import Mem0V3AutoCompact
...
class Mem0V3MemoryAlgorithm(MemoryAlgorithm):
    ...
    - Zero external dependencies beyond nanobot standard stack
    - Compatible with Dream and Hermes modes
    - Embedding-based semantic search via provider.embed()

    Configuration is controlled via nanobot's standard config:
