...
if TYPE_CHECKING:
    from nanobot.config.schema import ProxyPoolConfig
...
_USER_AGENT = "Mozilla/5.0 (compatible; nanobot-proxy-pool/1.0)"
    ...
    def __init__(self, config: ProxyPoolConfig) -> None:
        ...
        if config.proxy_cache_enabled:
            if config.proxy_cache_path:
                ...
                self._cache_path = Path.home() / ".nanobot" / "proxy_cache.json"
