...
`heartbeat` 模块为 nanobot agent 提供**周期性后台唤醒**机制。它定时读取 workspace 下的 `HEARTBEAT.md` 任务清单，借助 LLM 判断是否有待执行的任务，并在有任务时自动触发完整 agent 循环执行，最终将结果投递到用户的活跃聊天频道。
...
### Phase 3 — 通知评估（与 `nanobot.utils.evaluator` 协同）
...
心跳服务在 **Gateway 模式**下通过 CLI 命令 `nanobot gateway` 启动（见 `nanobot/cli/commands.py`）：
...
| 模块 | 关系 |
|---|---|
| `nanobot/utils/evaluator.py` | Phase 3 通知评估，共享相同的 tool-call 模式 |
| `nanobot/agent/loop.py` | Phase 2 通过 `agent.process_direct()` 执行任务 |
| `nanobot/config/schema.py` | `HeartbeatConfig` 配置定义 |
| `nanobot/cli/commands.py` | Gateway 入口，组装并启动 HeartbeatService |
| `nanobot/bus/events.py` | `OutboundMessage` 事件，用于通知投递 |
