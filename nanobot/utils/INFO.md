# nanobot/utils — 通用工具模块

## 概述

`nanobot/utils` 是 nanobot Agent 框架的基础工具模块，提供一系列零业务耦合的通用功能，覆盖路径展示、文档解析、Git 版本控制、提示模板渲染、Token 估算、工具调用格式化、运行时保护、重启通知、搜索用量查询、以及后执行评估等场景。模块内的各组件被 Agent 执行管道、CLI、Channel、Memory、Heartbeat 等多个上层模块广泛复用。
...
| `maybe_persist_tool_result(...)` | 超大工具输出持久化到磁盘 | 超过 `max_chars` 时写入 `.nanobot/tool-results/`，返回引用文本 |
...
**`.gitignore` 策略**：默认忽略所有文件（`/*`），仅白名单追踪文件和 `.gitignore` 自身。所有提交使用统一身份 `nanobot <nanobot@dream>`。

### 5. `prompt_templates.py` — 提示模板渲染

基于 Jinja2 的模板引擎，用于渲染 Agent 系统提示（位于 `nanobot/templates/`）。
...
处理 nanobot 进程重启后的跨进程通知（通过环境变量传递）。
...
`nanobot/utils` 被以下模块广泛引用：

| 上层模块 | 引用的 utils 组件 |
|----------|-------------------|
| `nanobot/agent/loop.py` | `runtime` (repeated_external_lookup_error, ensure_nonempty_tool_result), `tool_hints`, `helpers` (token 估算), `evaluator` |
| `nanobot/agent/context.py` | `helpers` (maybe_persist_tool_result, estimate_prompt_tokens_chain) |
| `nanobot/agent/runner.py` | `runtime` (build_finalization_retry_message, build_length_recovery_message) |
| `nanobot/agent/planner.py` | `prompt_templates` (render_template) |
| `nanobot/session/manager.py` | `restart` (consume_restart_notice_from_env) |
| `nanobot/cli/stream.py` | `tool_hints` (format_tool_hints), `path` (abbreviate_path) |
| `nanobot/cli/__init__.py` | `restart` (should_show_cli_restart_notice) |
| `nanobot/channels/` (各 channel) | `helpers` (split_message, truncate_text), `restart` |
| `nanobot/heartbeat/service.py` | `evaluator` (evaluate_response) |
| `nanobot/cron/service.py` | `evaluator` (evaluate_response) |
| `nanobot/nanobot.py` | `helpers` (build_status_content), `searchusage` (fetch_search_usage) |
| `nanobot/memory/` (各算法) | `gitstore` (GitStore), `helpers` (ensure_dir, sync_workspace_templates) |
