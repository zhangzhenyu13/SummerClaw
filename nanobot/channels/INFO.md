# nanobot/channels — 聊天通道模块

## 概述

`nanobot/channels` 是 nanobot 的**多平台聊天通道适配层**，采用插件架构为 Agent 提供与 14 种聊天平台的无缝集成能力。每个通道以统一的 `BaseChannel` 接口与消息总线（`MessageBus`）交互，使 Agent 核心完全无需感知消息来源平台的差异。
...
- **内置通道发现**（`discover_channel_names`）：使用 `pkgutil.iter_modules()` 扫描 `nanobot/channels/` 包，排除 `base`、`manager`、`registry`
- **通道类加载**（`load_channel_class`）：动态导入模块，反射查找第一个 `BaseChannel` 子类
- **外部插件发现**（`discover_plugins`）：通过 `entry_points(group="nanobot.channels")` 加载第三方通道插件
...
通过 `entry_points` 机制注册第三方通道，无需修改 nanobot 源码：
...
| 模块 | 关系 |
|------|------|
| `nanobot/bus/` | 消息总线，Channel 通过 `MessageBus` 与 Agent 核心通信 |
| `nanobot/config/schema.py` | `ChannelsConfig` 全局通道配置 + 各通道 `XXXConfig` 定义 |
| `nanobot/config/paths.py` | 媒体文件目录、运行时子目录、Bridge 安装路径 |
| `nanobot/providers/transcription.py` | 语音转写服务（Groq / OpenAI Whisper） |
| `nanobot/utils/helpers.py` | `split_message` 长文本分块工具 |
| `nanobot/security/network.py` | `validate_url_target` URL 安全性校验 |
| `nanobot/cli/commands.py` | CLI 中 `channels login` / `channels status` 等管理命令 |
| `nanobot/cli/onboard.py` | 初始化向导中生成各通道默认配置 |
