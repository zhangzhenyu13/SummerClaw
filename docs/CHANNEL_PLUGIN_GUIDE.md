...
Build a custom summerclaw channel in three steps: subclass, package, install.

> **Note:** We recommend developing channel plugins against a source checkout of summerclaw (`pip install -e .`) rather than a PyPI release, so you always have access to the latest base-channel features and APIs.

## How It Works

summerclaw discovers channel plugins via Python [entry points](https://packaging.python.org/en/latest/specifications/entry-points/). When `summerclaw gateway` starts, it scans:

1. Built-in channels in `summerclaw/channels/`
2. External packages registered under the `summerclaw.channels` entry point group
...
```
summerclaw-channel-webhook/
├── summerclaw_channel_webhook/
...
```python
# summerclaw_channel_webhook/__init__.py
from summerclaw_channel_webhook.channel import WebhookChannel

__all__ = ["WebhookChannel"]
```

```python
# summerclaw_channel_webhook/channel.py
import asyncio
...
from summerclaw.config.schema import Base
...
# pyproject.toml
[project]
name = "summerclaw-channel-webhook"
version = "0.1.0"
dependencies = ["summerclaw-ai", "aiohttp"]

[project.entry-points."summerclaw.channels"]
webhook = "summerclaw_channel_webhook:WebhookChannel"
...
packages = ["summerclaw_channel_webhook"]
```
...
```bash
pip install -e .
summerclaw plugins list      # verify "Webhook" shows as "plugin"
summerclaw onboard           # auto-adds default config for detected plugins
```

Edit `~/.summerclaw/config.json`:
...
```bash
summerclaw channels login <channel_name>
summerclaw channels login <channel_name> --force  # re-authenticate
```
...
| `default_config()` (classmethod) | Returns default config dict for `summerclaw onboard`. Override to declare your fields. |
...
Built-in channels use Pydantic config models (subclassing `Base` from `summerclaw.config.schema`). Plugin channels **must do the same**.

### Pattern

1. Define a Pydantic model inheriting from `summerclaw.config.schema.Base`:
...
Override `default_config()` so `summerclaw onboard` auto-populates `config.json`:
...
| What | Format | Example |
|------|--------|---------|
| PyPI package | `summerclaw-channel-{name}` | `summerclaw-channel-webhook` |
| Entry point key | `{name}` | `webhook` |
| Config section | `channels.{name}` | `channels.webhook` |
| Python package | `summerclaw_channel_{name}` | `summerclaw_channel_webhook` |

## Local Development

```bash
git clone https://github.com/you/summerclaw-channel-webhook
cd summerclaw-channel-webhook
pip install -e .
summerclaw plugins list    # should show "Webhook" as "plugin"
summerclaw gateway         # test end-to-end
```

## Verify

```bash
$ summerclaw plugins list
