# summerclaw/api — OpenAI-compatible HTTP API Module

## 模块概述

`summerclaw/api` 模块为 summerclaw 提供了一套 **OpenAI API 兼容的 HTTP 服务**，使得任何兼容 OpenAI SDK 的客户端（如 openai-python、LangChain、Continue.dev 等）都能直接调用 summerclaw 的 Agent 能力。所有请求路由到同一个持久化的 `AgentLoop` 会话。

## 文件结构

| 文件 | 说明 |
|------|------|
| `__init__.py` | 模块入口，声明 `OpenAI-compatible HTTP API for summerclaw` |
...
{
  "object": "list",
  "data": [{"id": "summerclaw", "object": "model", "created": 0, "owned_by": "summerclaw"}]
}
```
...
`create_app(agent_loop, model_name="summerclaw", request_timeout=120.0)` 创建 aiohttp 应用实例：

| 参数 | 默认值 | 说明 |
|------|--------|------|
| `agent_loop` | *(必填)* | 已初始化的 `AgentLoop` 实例 |
| `model_name` | `"summerclaw"` | API 响应中报告的模型名 |
...
```bash
summerclaw serve --port 8900 --host 127.0.0.1 --timeout 120
```
...
7. **依赖 aiohttp** — 需通过 `pip install 'summerclaw-ai[api]'` 安装
...
| 模块 | 关系 |
|------|------|
| `summerclaw.agent.loop` | API 调用 `AgentLoop.process_direct()` 处理请求 |
| `summerclaw.cli.commands` | `serve` 命令组装并启动 API 服务 |
| `summerclaw.config.schema` | `ApiConfig` 定义 API 配置项 |
| `summerclaw.utils.runtime` | 提供 `EMPTY_FINAL_RESPONSE_MESSAGE` 兜底消息 |
| `summerclaw.utils.evaluator` | `evaluate_response()` 用于心跳执行后的通知评估 |
| `summerclaw.utils.helpers` | `safe_filename()`、`current_time_str()` 辅助函数 |