# summerclaw/templates — 代理提示词模板与工作区脚手架

## 模块概述

`summerclaw/templates` 是 summerclaw agent 系统的**提示词模板引擎核心**。它包含两大部分：
...
所有模板由 `summerclaw/utils/prompt_templates.py` 中的 `render_template()` 函数统一加载和渲染。
...
### HEARTBEAT.md
定义周期性后台任务。summerclaw 按配置的间隔检查此文件，执行其中的活跃任务。
...
summerclaw 的 agent 执行分为多个决策阶段，每个阶段对应一个专门的提示词模板：
...
```python
# summerclaw/utils/prompt_templates.py
...
| 调用方 | 渲染的模板 |
|--------|-----------|
| `summerclaw/agent/context.py::ContextBuilder._build_system_prompt()` | `agent/identity.md`, `agent/platform_policy.md` |
| `summerclaw/agent/context.py::ContextBuilder.build_messages()` | `agent/skills_section.md` |
| `summerclaw/agent/complexity.py` | `agent/complexity_classifier_system.md` |
| `summerclaw/agent/search_planner.py` | `agent/search_decision_system.md`, `agent/search_agent_system.md` |
| `summerclaw/agent/planner.py` | `agent/planner_system.md` |
| `summerclaw/agent/runner.py` | `agent/subagent_announce.md` |
| `summerclaw/agent/subagent.py` | `agent/subagent_system.md` |
| `summerclaw/agent/evaluator.py` | `agent/evaluator_system.md` |
| `summerclaw/utils/evaluator.py` | `agent/evaluator.md` |
| `summerclaw/agent/hook.py`（Dream） | `agent/dream_phase1.md`, `agent/dream_phase2.md` |
| `summerclaw/agent/skill_autogen.py` | `agent/skill_autogen_review.md` |
| `summerclaw/memory/...`（各算法） | `agent/consolidator_archive.md` |
...
```python
# summerclaw/utils/helpers.py::sync_workspace_templates()

def sync_workspace_templates(workspace: Path, silent: bool = False) -> list[str]:
    """Sync bundled templates to workspace. Only creates missing files."""
```

该函数通过 `importlib.resources.files("summerclaw") / "templates"` 访问打包后的模板资源，在 workspace 初始化或首次启动时调用：
