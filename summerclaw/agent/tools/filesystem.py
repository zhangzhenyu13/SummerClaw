...
from summerclaw.agent.tools.base import Tool, tool_parameters
from summerclaw.agent.tools.schema import BooleanSchema, IntegerSchema, StringSchema, tool_parameters_schema
from summerclaw.agent.tools import file_state
from summerclaw.utils.helpers import build_image_content_blocks, detect_image_mime
from summerclaw.config.paths import get_media_dir
            ...
            if self._workspace:
                try:
                    ...
                    if rel_str.startswith("outputs" + os.sep):
                        from summerclaw.utils.outputs_meta import OutputMetaManager
...
class SkillPrefixWriteFileTool(WriteFileTool):
    """WriteFileTool that enforces a fixed prefix on skills/<name>/... paths.

    When the LLM writes to ``skills/<name>/SKILL.md`` (or any sub-path), this
    tool rewrites the path to ``skills/<prefix><name>/SKILL.md`` if the
    directory name does not already start with *prefix*.

    This hard-codes the naming convention so that even if the prompt is
    partially ignored the generated skill directory will always carry the
    correct origin prefix (e.g. ``dreamed-`` or ``hermes-``).
    """

    def __init__(self, skill_prefix: str, **kwargs: Any) -> None:
        super().__init__(**kwargs)
        if not skill_prefix.endswith("-"):
            skill_prefix = skill_prefix + "-"
        self._skill_prefix = skill_prefix

    def _enforce_prefix(self, path: str) -> str:
        """Rewrite skills/<name>/... → skills/<prefix><name>/... when needed."""
        import re
        m = re.match(r"^(skills/)([^/]+)(/.+)$", path)
        if m and not m.group(2).startswith(self._skill_prefix):
            return f"{m.group(1)}{self._skill_prefix}{m.group(2)}{m.group(3)}"
        return path

    async def execute(self, path: str | None = None, content: str | None = None, **kwargs: Any) -> str:
        if path:
            path = self._enforce_prefix(path)
