...
`summerclaw/agent` 是 SummerClaw/summerclaw 项目的**核心 Agent 引擎模块**，负责实现从消息接收、上下文构建、LLM 调用、工具执行到响应发送的完整 Agent 循环生命周期。该模块采用了 **ReAct（Reasoning + Acting）** 范式，并在此基础上扩展了 **Plan-and-Solve** 规划模式、**搜索增强规划**、**闭环评估与重规划**、**子代理（Subagent）后台执行**、**技能自动生成**以及**多模式前缀驱动执行**等高级能力。
...
```python
from summerclaw.agent import (
    AgentHook,        # 生命周期钩子基类
    AgentHookContext, # 钩子上下文数据类
    AgentLoop,        # 核心处理引擎
    CompositeHook,    # 组合钩子
    ContextBuilder,   # 上下文构建器
    Dream,            # 记忆固化（从 summerclaw.memory 重导出）
    MemoryStore,      # 记忆存储（从 summerclaw.memory 重导出）
    SkillsLoader,     # 技能加载器
