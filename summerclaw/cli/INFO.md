...
`summerclaw/cli` 是 summerclaw 的命令行交互层，基于 [Typer](https://typer.tiangolo.com/) 框架构建了完整的 CLI 应用入口。模块负责将所有底层能力（Agent 循环、Gateway、Channel 管理、配置管理等）以命令行形式暴露给用户，支持**单次对话**、**交互式聊天**、**初始化引导向导**、**API 服务管理**等多种使用模式。
...
### 内部依赖
- `summerclaw.agent.loop.AgentLoop` — Agent 核心循环
- `summerclaw.bus.queue.MessageBus` — 消息总线
- `summerclaw.config.loader` / `config.schema` — 配置加载与 Schema
- `summerclaw.config.paths` — 路径管理（workspace、history）
- `summerclaw.providers.registry` — LLM Provider 注册中心
- `summerclaw.channels.registry` / `channels.manager` — Channel 注册与管理
- `summerclaw.cron.service` — 定时任务服务
- `summerclaw.heartbeat.service` — 心跳服务
- `summerclaw.session.manager` — 会话管理
- `summerclaw.api.server` — OpenAI 兼容 API Server

## 核心命令一览

### 1. `summerclaw onboard` — 初始化配置

```bash
summerclaw onboard [--workspace <path>] [--config <path>] [--wizard]
```
...
### 2. `summerclaw agent` — Agent 对话

```bash
summerclaw agent -m "Hello!"                   # 单次对话
summerclaw agent                                # 交互式聊天（默认）
summerclaw agent --session cli:my-session       # 指定会话 ID
summerclaw agent --logs                         # 显示运行时日志
summerclaw agent --no-markdown                  # 纯文本输出
```
...
### 3. `summerclaw serve` — OpenAI 兼容 API 服务

```bash
summerclaw serve [--port <port>] [--host <host>] [--timeout <s>] [--verbose]
```
...
### 4. `summerclaw gateway` — 全功能网关

```bash
summerclaw gateway [--port <port>] [--verbose]
```
...
### 5. `summerclaw channels` — Channel 管理

```bash
summerclaw channels status          # 查看 Channel 状态（启用/禁用）
summerclaw channels login <name>    # 交互式登录（如 WhatsApp QR 码）
```

### 6. `summerclaw plugins` — 插件管理

```bash
summerclaw plugins list             # 列出所有发现的 Channel（内置 + 插件）
```

### 7. `summerclaw status` — 状态查看

显示当前配置状态：配置文件位置、Workspace、模型、Provider API Key 状态。

### 8. `summerclaw provider login` — OAuth 登录
