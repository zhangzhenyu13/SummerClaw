...
from summerclaw.memory.naive_memory.consolidator import Consolidator as _NaiveConsolidator
        ...
        if summary and self.has_hindsight:
            try:
                await self._hindsight_store.aretain(
                    content=summary,
                    context=f"summerclaw consolidation: {len(messages)} messages",
                )
                ...
                if self.has_hindsight and extracted_facts:
                    try:
                        for fact in extracted_facts:
                            await self._hindsight_store.aretain(
                                content=fact["memory"],
                                context="summerclaw hermes extraction",
                            )
                        logger.debug(
